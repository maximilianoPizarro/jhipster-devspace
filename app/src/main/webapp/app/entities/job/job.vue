<template>
  <div>
    <h2 id="page-heading" data-cy="JobHeading">
      <span v-text="t$('deliveryApp.job.home.title')" id="job-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('deliveryApp.job.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'JobCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-job">
            ...
            <span v-text="t$('deliveryApp.job.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && jobs && jobs.length === 0">
      <span v-text="t$('deliveryApp.job.home.notFound')"></span>
    </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="deliveryApp.job.delete.question" data-cy="jobDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-job-heading" v-text="t$('deliveryApp.job.delete.question', { id: removeId })"></p>
      </div>
